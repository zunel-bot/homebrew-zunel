...
- pluggable LLM providers — any OpenAI-compatible endpoint, ChatGPT Codex
  via OAuth, or Amazon Bedrock
...
No source code lives here; the source is maintained at
[zunel-bot/zunel](https://github.com/zunel-bot/zunel).
...
`zunel` ships three provider paths. Pick one by editing
`~/.zunel/config.json`.
...
**Option C — Amazon Bedrock (`providers.bedrock`):**

Uses the standard AWS credential chain (env vars, shared credentials
file, SSO profile, or instance role):
...
If you use AWS SSO, run `aws sso login --profile <your-profile>` once
per session before starting `zunel`.
